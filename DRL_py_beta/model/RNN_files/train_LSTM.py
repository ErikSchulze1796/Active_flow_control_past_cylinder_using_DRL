...
def train_LSTM(model, train_data, val_data, save_best_dir: str, batch_size: int=64, n_epochs: int=1, learning_rate: float=0.001):
    """Train an LSTM as
    ...
    for epoch in range(n_epochs):
        ...
        for k, (feature_batch, label_batch) in enumerate(train_loader):
            ...
            optimizer.zero_grad()
            # Get prediction per training feature batch
        ...
        with torch.no_grad():
            ...
            for j, (feature_batch_val, label_batch_val) in enumerate(val_loader):
                # Get prediction per validation feature batch
        ...
        print("Computation times (t_epoch|t_train|t_val): {} | {} | {}".format(t_epoch, t_train, t_val))
        print("# =========================================== #")
        ...
        if isdir(save_best_dir):
            if train_loss[-1] < best_train_loss:
                torch.save(model.state_dict(), f"{save_best_dir}_LSTM_best_model_train_{learning_rate}_n_history{n_steps_history}.pt")
                best_train_loss = train_loss[-1]
                
            if val_loss[-1] < best_val_loss:
                torch.save(model.state_dict(), f"{save_best_dir}_LSTM_best_model_val_{learning_rate}_n_history{n_steps_history}.pt")
                best_val_loss = val_loss[-1]
        
        # Save training and validation loss to pytorch binary file
        torch.save(train_loss, f"{save_best_dir}_LSTM_train_loss_{learning_rate}_n_history{n_steps_history}.pt")
        torch.save(val_loss, f"{save_best_dir}_LSTM_val_loss_{learning_rate}_n_history{n_steps_history}.pt")
