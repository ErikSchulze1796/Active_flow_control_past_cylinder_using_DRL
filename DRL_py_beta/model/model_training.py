...
def optimize_model(model: pt.nn.Module, train_loader, val_loader,
    ...
    for e in range(1, epochs+1):
        ...
        with pt.no_grad():
            for j, (feature_batch_val, label_batch_val) in enumerate(val_loader):
                ...
                loss = criterion(prediction, label_batch_val)
        ...
        if isdir(save_best):
            if train_loss[-1] < best_train_loss:
                pt.save(model.state_dict(), f"{save_best}{prefix}best_model_train_{lr}_n_history{n_steps_history}_neurons{n_neurons}_layers{n_layers}.pt")
                best_train_loss = train_loss[-1]
                
            if val_loss[-1] < best_val_loss:
                pt.save(model.state_dict(), f"{save_best}{prefix}best_model_val_{lr}_n_history{n_steps_history}_neurons{n_neurons}_layers{n_layers}.pt")
